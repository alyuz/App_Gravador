...
class _GravadorPageState extends State<GravadorPage> {
  ...
  Future<void> _saveRecording() async {
    ...
    String formatFileName(DateTime datetime) {
      final year = datetime.year.toString().padLeft(4, '0');
      final month = datetime.month.toString().padLeft(2, '0');
      final day = datetime.day.toString().padLeft(2, '0');
      final hour = datetime.hour.toString().padLeft(2, '0');
      final minute = datetime.minute.remainder(60).toString().padLeft(2, '0');
      final second = datetime.second.remainder(60).toString().padLeft(2, '0');
      final millisecond = datetime.millisecond.remainder(1000).toString().padLeft(4, '0');

      return '${year}_${month}_${day}__${hour}_${minute}_${second}_$millisecond';
    }

    final date = formatFileName(DateTime.now());

    final directory = await getApplicationDocumentsDirectory();
    final savedFile = File(
      '${directory.path}/audio_salvo_$date.m4a',
    );

    final user = FirebaseAuth.instance.currentUser;
    final storageRef = FirebaseStorage.instance.ref('$user/recordings/$date.m4a');
    final filePathFirebase = storageRef.fullPath;
    ...
    setState(() {
      ...
      _filePathFirebase = filePathFirebase;
      _savedPath = savedFile.path;
      _pathToDrop = savedFile;
    });
    _showMessage('Audio salvo no Firebase: $_downloadUrl.');
  }
  ...
  Future <void> _dropRecording() async {
    final path = _filePathFirebase;
    final file = _pathToDrop;

    try {
      FirebaseStorage.instance.ref(path).delete();
      _showMessage('Gravação apagada do firebase');
    } catch (e) {
      _showMessage('Erro ao apagar gravação do firebase: $e');
    }

    try {
      file!.delete();
      _showMessage('Gravação apagada do dispositivo');
    } catch (e) {
      _showMessage('Erro ao apagar gravação do dispositivo: $e');
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                onPressed: hasRecording ? (_savedPath == null ? _saveRecording : null) : null,
              ...
              const SizedBox(height: 12),
              TextButton.icon(
                // Deletar gravacao fica disponivel quando ha algo para limpar
                onPressed: (hasRecording)
                    ? _dropRecording
                    : null,
                icon: const Icon(Icons.delete),
                label: const Text('Deletar gravacao'),
              ),
              const Spacer(),
              if (_savedPath != null)
